...
public:
	...
	void Add(HWND hwnd, CSubclassWnd* pMsgHook);
	void Remove(CSubclassWnd* pMsgHook);
...
HookWndProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
	...
	CSubclassWnd* pMsgHook = theHookMap.Lookup(hwnd);
	ASSERT(pMsgHook);
...
//
void CSubclassWndMap::Add(HWND hwnd, CSubclassWnd* pMsgHook)
{
	ASSERT(hwnd && ::IsWindow(hwnd));
