...
static list findalldevs()
{
	...
	{
		PyErr_SetString( PyExc_RuntimeError,  gErrbuf);
		throw_error_already_set();
	}
...
static int sendTo( int fd, object o, tuple t )
{
	std::string addr = extract<std::string>( t[0] );
	int port = extract<int>( t[1] );
	const void * buf;
	Py_ssize_t l;
	PyObject_AsReadBuffer ( o.ptr(), &buf, &l );
	
	SOCKADDR_IN addr_in = {0};
	addr_in.sin_family  = AF_INET;
	addr_in.sin_port = htons( port );
	addr_in.sin_addr.S_un.S_addr =  inet_addr( addr.c_str() );
	int ret = sendto(fd, (char*)buf,l, 0, (SOCKADDR *)&addr_in, sizeof(addr_in));
	if( ret <1 ){
...
static int rawSocket(){
	int sock = WSASocket(AF_INET,SOCK_RAW,IPPROTO_UDP,NULL,0,0);
	if( sock <1 ){
		PyErr_SetFromWindowsErr( GetLastError() );
		throw_error_already_set();
	}
